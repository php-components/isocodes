...
class Json extends AbstractAdapter implements AdapterInterface
{
    ...
    public function get($code)
    {
        ...
        if (is_numeric($code)) {
            ...
            foreach ($this->data as $current) {
                if (strcasecmp($current->alpha3, $code) === 0) {
                    return $current;
                }
            }
        } else {
    ...
    {
        $filename = dirname(dirname(dirname(__DIR__))) . '/data/json/iso_3166-1.json';
        ...
        if (!array_key_exists('3166-1' , $data)) {
            throw new Exception\RuntimeException(sprintf('%s is not a valid JSON file for ISO-3166-1.', $filename));
        }
        ...
        foreach ($data as $current) {
            ...
            $this->data[strtoupper($current['alpha_2'])] = $obj; 
        }
    }
}
